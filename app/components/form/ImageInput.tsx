...
const ImageInput: React.FC<Props> = ({
  ...
  return (
    <div>
      {!multiple && (
        <div className="mb-4 flex items-center gap-3">
          <Avatar className="h-20 w-20 rounded-full">
            <AvatarImage src={value[0]?.src} alt="Image input" />
          ...
          <div className="flex flex-col ">
            <span className="mb-1.5 text-sm">
              {value[0]?.name ||
                (value[0]?.origin === "remote" ? value[0].src : undefined) ||
                "Choose file"}
            </span>
            {value[0] && (
              <Trash2
      ...
      >
        <input
          ...
          onChange={async (event) => {
            ...
            const results: FormFile[] = await Promise.all(
              Array.from(files).map((f) => FormFile.fromFile(f))
            );
      ...
      {multiple && (
        <div className={clsx(`flex gap-3 bg-accent my-5 overflow-x-auto`)}>
          {value?.map((image, index) => (
            <Image
              className="w-40 h-40 hover:opacity-30 object-cover"
              onClick={() => onChange(value.filter((val) => val !== image))}
              src={image.src}
              key={index}
              width={500}
              height={500}
              alt={
                image.name ||
                (image.origin === "remote" ? image.src : undefined) ||
                `File ${index}`
              }
            />
          ))}
